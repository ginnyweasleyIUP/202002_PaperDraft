...
for(run in c("a","b","c")){
  ...
  plot_c_sim <- ANALYSIS$NETWORK[[paste0("GLOBAL_SIM_ds",run)]]$C
  plot_c_sim[lower.tri(ANALYSIS$NETWORK[[paste0("GLOBAL_SIM_ds",run)]]$C, diag = FALSE)] = NA
  lowess_c_sim <- as.vector(ANALYSIS$NETWORK[[paste0("GLOBAL_SIM_ds",run)]]$C[upper.tri(ANALYSIS$NETWORK[[paste0("GLOBAL_SIM_ds",run)]]$C)])
  ...
  pdf(file = paste0("Plots/Paper_Plot_6_Network_a_ds_xnap",run,".pdf"), height= PLOTTING_VARIABLES$HEIGHT, width = PLOTTING_VARIABLES$WIDTH)
