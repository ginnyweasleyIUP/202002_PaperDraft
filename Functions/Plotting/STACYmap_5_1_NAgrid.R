...
STACYmap_NA <- function(gridlyr = NULL,
                     NA_gridlyr = NULL, 
                     ptlyr = NULL, 
                     ptlyr_size_var = NULL,
  ...
  #NA_gridlyr
  if (!is.null(NA_gridlyr)) {
    if (!any(class(NA_gridlyr) %in% c('raster', 'matrix', 'data.frame'))) {
      stop('class(gridlyr) has to be one of \'raster\', \'matrix\', \'data.frame\'')
    } else {
      prepg <- list(raster = function(x) x, 
                    matrix = function(x) {
      ...
      NA_gridlyr <- NA_gridlyr %>% 
        prepg[[class(NA_gridlyr)]](.) %>% 
        raster::rasterToPolygons() %>% 
        spTransform(CRS(projection)) %>%
        as('SpatialPolygonsDataFrame') %>% 
        as('sf')
  ...
  if(!is.null(NA_gridlyr)){
    map_plot <- map_plot + 
      geom_sf(data = NA_gridlyr, color = NA_color)
  }
