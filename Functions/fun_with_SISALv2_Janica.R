...
load_sisal_data_janica <- function(prefix = "", path = "/stacywork/ginnyweasley/02_SISAL/SISAL_v2/", year_start, year_stop){
  ...
  entities_mineralogy <- sample_tb_new_filtered %>% filter(interp_age < (1950-year_start) & interp_age > (1950-year_stop)) %>% distinct(entity_id, mineralogy)
  entities_mineralogy <- sample_tb_new_filtered %>% filter(interp_age < (1950-year_start) & interp_age > (1950-year_stop)) %>% distinct(entity_id, mineralogy) %>% filter(entity_id != 107, entity_id != 188, entity_id != 190, entity_id != 197,
  ...
  entities_dating <- sample_tb_new_filtered %>% filter(interp_age<(1950-year_start) & interp_age > (1950-year_stop)) %>% distinct(entity_id, age_model_type) %>% right_join(., dating_join, by = "entity_id") %>% distinct(entity_id, date_type, age_model_type)
  ...
  entities_past1000 <- sample_tb_new_filtered %>% filter(interp_age < (1950-year_start) & interp_age > (1950-year_stop)) %>% group_by(entity_id) %>% count() %>% 
