...
for(plot in 1:1){
  ...
  plot_c_sim <- c_ensemble
  plot_c_sim[lower.tri(c_ensemble, diag = FALSE)] = NA
  lowess_c_sim <- as.vector(c_ensemble[upper.tri(c_ensemble)])
  ...
  pdf(file = paste0("Plots/Paper_Plot_6_Network_a_ensemble.pdf"), height= PLOTTING_VARIABLES$HEIGHT, width = PLOTTING_VARIABLES$WIDTH)
