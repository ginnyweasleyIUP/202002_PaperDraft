...
for(cluster in 1:9){
  ...
  ## mask 50% closest variance ratio
  entity_total <- ANALYSIS$NETWORK$entity_meta$entity_id
  cluster_list <- ANALYSIS$NETWORK$entity_meta$entity_id[ANALYSIS$NETWORK$entity_meta$cluster_id == cluster]
  index.c <- length(cluster_list)
  offset = DISCUSSION$scatter_data_variance %>% filter(entity_id %in% cluster_list)
  #offset = scatter_data %>% filter(entity_id %in% cluster_list)
  offset = offset$vr_ds_itpc
  cluster_list[abs(offset-1)>median(abs(offset-1), na.rm = T)] = NA
  cluster_list = na.omit(cluster_list)
  for(ii in 1:length(cluster_list)){
    index.c[ii] = which(cluster_list[ii] == entity_total)
  }
  ...
  DISCUSSION$NETWORK_TUNING$CLUSTER[[paste0("CLUSTER",cluster)]]$smallest_var <- list()
  DISCUSSION$NETWORK_TUNING$CLUSTER[[paste0("CLUSTER",cluster)]]$smallest_var$C_rec <- ANALYSIS$NETWORK$GLOBAL$C[mask]
  DISCUSSION$NETWORK_TUNING$CLUSTER[[paste0("CLUSTER",cluster)]]$smallest_var$P_rec <- ANALYSIS$NETWORK$GLOBAL$P[mask]
  DISCUSSION$NETWORK_TUNING$CLUSTER[[paste0("CLUSTER",cluster)]]$smallest_var$C_rec_gauss <- ANALYSIS$NETWORK$GLOBAL$C_gauss[mask]
  DISCUSSION$NETWORK_TUNING$CLUSTER[[paste0("CLUSTER",cluster)]]$smallest_var$P_rec_gauss <- ANALYSIS$NETWORK$GLOBAL$P_gauss[mask]
  
  DISCUSSION$NETWORK_TUNING$CLUSTER[[paste0("CLUSTER",cluster)]]$smallest_var$C_rec_ensemble <- c_ensemble[mask]
  DISCUSSION$NETWORK_TUNING$CLUSTER[[paste0("CLUSTER",cluster)]]$smallest_var$C_rec_chrono <- ANALYSIS$NETWORK$GLOBAL_CHRONO$C_max[mask]
  
  ## mask only same seasons
  mask = matrix(logical(length = 89*89), ncol = 89) 
  for(season in 1:5){
    entity_total <- ANALYSIS$NETWORK$entity_meta$entity_id
    cluster_list <- ANALYSIS$NETWORK$entity_meta$entity_id[ANALYSIS$NETWORK$entity_meta$cluster_id == cluster]
    index.c <- length(cluster_list)
    offset = ANALYSIS$SEASONS$Plot_Lyr_ISOT_a %>% filter(entity_id %in% cluster_list)
    #offset = scatter_data %>% filter(entity_id %in% cluster_list)
    offset = offset$season
    cluster_list[offset != season] = NA
    cluster_list[is.na(offset)] = NA
    cluster_list = na.omit(cluster_list)
    if(length(cluster_list) == 0){next}
    for(ii in 1:length(cluster_list)){
      index.c[ii] = which(cluster_list[ii] == entity_total)
    }
    mask_new = matrix(logical(length = 89*89), ncol = 89)
    
    for(ii in 1:(length(index.c)-1)){
      for(jj in ii:length(index.c)){
        mask_new[index.c[ii],index.c[jj]] = T
        mask_new[index.c[jj],index.c[ii]] = T
        mask_new[index.c[ii],index.c[ii]] = T
        mask_new[index.c[jj],index.c[jj]] = T
      }
    }
    mask = mask + mask_new
  }
  
  mask_log = matrix(logical(length = 89*89), ncol = 89) 
  
  mask_log[mask == 1] = T
  mask = mask_log
  
  DISCUSSION$NETWORK_TUNING$CLUSTER[[paste0("CLUSTER",cluster)]]$same_season <- list()
  DISCUSSION$NETWORK_TUNING$CLUSTER[[paste0("CLUSTER",cluster)]]$same_season$C_rec <- ANALYSIS$NETWORK$GLOBAL$C[mask]
  DISCUSSION$NETWORK_TUNING$CLUSTER[[paste0("CLUSTER",cluster)]]$same_season$P_rec <- ANALYSIS$NETWORK$GLOBAL$P[mask]
  DISCUSSION$NETWORK_TUNING$CLUSTER[[paste0("CLUSTER",cluster)]]$same_season$C_rec_gauss <- ANALYSIS$NETWORK$GLOBAL$C_gauss[mask]
  DISCUSSION$NETWORK_TUNING$CLUSTER[[paste0("CLUSTER",cluster)]]$same_season$P_rec_gauss <- ANALYSIS$NETWORK$GLOBAL$P_gauss[mask]
...
col_same_season <- c("#6A51A3", "#9E9AC8", "#CBC9E2", "#F2E6F7")
col_offset <- c("#CB181D", "#FB6A4A", "#FCAE91", "#FEE5D9")
for(plot in 1:1){
  pdf(file = paste0("Plots/Discussion/Network_c_boxplot_extended_2.pdf"), width = 21, height = 29.7)
  par(mar = c(2,9,8,2))
  plot(c(-1,1), c(1,59+13), type = "n", axes = FALSE, xlab = "", ylab = "" )
  abline(v=0)
  ## SITES
  ...
  for(cluster in c(6,2,3,7,1,5,9)){
    corr <- list(full = as.numeric(DISCUSSION$NETWORK_TUNING$CLUSTER[[paste0("CLUSTER",cluster)]]$C_rec[DISCUSSION$NETWORK_TUNING$CLUSTER[[paste0("CLUSTER",cluster)]]$P_rec<0.1]), 
                 ...
                 chrono = as.numeric(DISCUSSION$NETWORK_TUNING$CLUSTER[[paste0("CLUSTER",cluster)]]$C_rec_chrono),
                 ensemble = as.numeric(DISCUSSION$NETWORK_TUNING$CLUSTER[[paste0("CLUSTER",cluster)]]$C_rec_ensemble),
                 same_season_full = as.numeric(DISCUSSION$NETWORK_TUNING$CLUSTER[[paste0("CLUSTER",cluster)]]$same_season$C_rec[DISCUSSION$NETWORK_TUNING$CLUSTER[[paste0("CLUSTER",cluster)]]$same_season$P_rec<0.1]),
                 same_season_gauss = as.numeric(DISCUSSION$NETWORK_TUNING$CLUSTER[[paste0("CLUSTER",cluster)]]$same_season$C_rec_gauss[DISCUSSION$NETWORK_TUNING$CLUSTER[[paste0("CLUSTER",cluster)]]$same_season$P_rec_gauss<0.1]),
                 same_season_chrono = as.numeric(DISCUSSION$NETWORK_TUNING$CLUSTER[[paste0("CLUSTER",cluster)]]$same_season$C_rec_chrono),
                 same_season_ensemble = as.numeric(DISCUSSION$NETWORK_TUNING$CLUSTER[[paste0("CLUSTER",cluster)]]$same_season$C_rec_ensemble),
                 
                 offset_full = as.numeric(DISCUSSION$NETWORK_TUNING$CLUSTER[[paste0("CLUSTER",cluster)]]$smallest_var$C_rec[DISCUSSION$NETWORK_TUNING$CLUSTER[[paste0("CLUSTER",cluster)]]$smallest_var$P_rec<0.1]),
                 offset_gauss = as.numeric(DISCUSSION$NETWORK_TUNING$CLUSTER[[paste0("CLUSTER",cluster)]]$smallest_var$C_rec_gauss[DISCUSSION$NETWORK_TUNING$CLUSTER[[paste0("CLUSTER",cluster)]]$smallest_var$P_rec_gauss<0.1]),
                 offset_chrono = as.numeric(DISCUSSION$NETWORK_TUNING$CLUSTER[[paste0("CLUSTER",cluster)]]$smallest_var$C_rec_chrono),
                 offset_ensemble = as.numeric(DISCUSSION$NETWORK_TUNING$CLUSTER[[paste0("CLUSTER",cluster)]]$smallest_var$C_rec_ensemble),
    ...
    boxplot(corr$same_season_full, add = TRUE, at = position[[paste0("cluster",cluster)]][5] , boxwex = 1, names = "n", horizontal = T, col = col_same_season[1], axes = F, outline = F)
    boxplot(corr$same_season_gauss, add = TRUE, at = position[[paste0("cluster",cluster)]][6] , boxwex = 1, names = "n", horizontal = T, col = col_same_season[2], axes = F, outline = F)
    boxplot(corr$same_season_chrono, add = TRUE, at = position[[paste0("cluster",cluster)]][7] , boxwex = 1, names = "n", horizontal = T, col = col_same_season[3], axes = F, outline = F)
    boxplot(corr$same_season_ensemble, add = TRUE, at = position[[paste0("cluster",cluster)]][8] , boxwex = 1, names = "n", horizontal = T, col = col_same_season[4], axes = F, outline = F)
  ...
  legend(-1.075,72.5+3.5,xpd = T,inset=-0.2, bty='n', x.intersp=0.5,text.width=c(1,0.25,0.4, 0.37),
         c("record","record 100gauss","xnap(a/b/c)", "xnap(a/b/c) 100gauss"), fill=c("white", "grey", "dodgerblue3", adjustcolor("dodgerblue3", alpha.f = 0.5)), horiz=TRUE, cex=1)
  legend(-1.075,72.5+4.5,xpd = T,inset=-0.2, bty='n', x.intersp=0.5,text.width=c(1,0.25,0.4, 0.37),
         c("same_season record","same_season record 100gauss","same_season record chrono", "same_season record ensemble"), fill=col_same_season, horiz=TRUE, cex=1)
  legend(-1.075,72.5+5.5,xpd = T,inset=-0.2, bty='n', x.intersp=0.5,text.width=c(1,0.25,0.4, 0.37),
         c("var record","var record 100gauss","var record chrono", "var record ensemble"), fill=col_offset, horiz=TRUE, cex=1)
  legend(-1.075,72.5+6.5,xpd = T,inset=-0.2, bty='n', x.intersp=0.5,text.width=c(1,0.25,0.4, 0.37),
         c("record chrono", "record ensemble"), fill=c(col_chrono, col_ensemble), horiz=TRUE, cex=1)
