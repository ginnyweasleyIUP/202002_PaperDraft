...
load_sisal_data_janica <- function(prefix = "", path = "/home/ginnyweasley/Dokumente/01_Promotion/06_Daten/02_SISAL/SISAL_v2_CARLA/"){
  prefix = ""
  ...
  entities_mineralogy <- sample_tb_new_filtered %>% filter(interp_age < 1100) %>% distinct(entity_id, mineralogy)
  entities_mineralogy <- sample_tb_new_filtered %>% filter(interp_age < 1100) %>% distinct(entity_id, mineralogy) %>% filter(entity_id != 107, entity_id != 188, entity_id != 190, entity_id != 197,
                                                                                                                entity_id != 348, entity_id != 349, entity_id != 436)# %>% group_by(entity_id) %>% count()
  ...
  entities_dating <- sample_tb_new_filtered %>% filter(interp_age<1100) %>% distinct(entity_id, age_model_type) %>% right_join(., dating_join, by = "entity_id") %>% distinct(entity_id, date_type, age_model_type)
  ...
  entities_past1000 <- sample_tb_new_filtered %>% filter(interp_age < 1100) %>% group_by(entity_id) %>% count() %>% 
  ...
  sample_min2 <- sample_tb_new_filtered %>% filter(entity_id %in% entities_past1000$entity_id) %>% distinct(entity_id, interp_age, d18O_measurement) %>% filter(interp_age<1100)
...
return(list(return_data, sample_min2, entities_past1000_dating, site_to_entity))
  ...
  remove(composite_link_entity, d13C, d18O, dating, dating_lamina, dating_tb, dating_tb_filtered, dating_tb_filtered_2, dating_tb_new, 
         ...
         entities_dating, entities_past1000_dating)
