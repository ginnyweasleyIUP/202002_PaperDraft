...
for(run in c(a,b,c)){
# für xnapb und xnapc kann man dann ja einen eigenen Datensatz DATA_past1000$SIM_YEARLY_A/B/C machen?
  DATA_past1000 <- list()
...
#begins 216375 which is days since 2350-12-01 --> first month is mid december
ncf<-ncdf4::nc_open("/stacywork/hadcm3/surface_temperature/monthly_fixed/xnapa.nc")
DATA_past1000_SIM_RAW$TEMP <- clear_data_matrix(ncdf4::ncvar_get(ncf),1)
ncdf4::nc_close(ncf)

#starts at same date
ncf<-ncdf4::nc_open("/stacywork/hadcm3/precipitation/monthly_fixed/xnapa.nc")
...
#starts at same date
ncf<-ncdf4::nc_open("/stacywork/hadcm3/isotopes/monthly_fixed/xnapa.nc")
ncf2<-ncdf4::nc_open("/stacywork/ginnyweasley/05_HadCM3/xnap/xnapa_isotopes.nc")
DATA_past1000_SIM_RAW$ISOT <- clear_data_matrix(ncdf4::ncvar_get(ncf, 'dO18'),3)
ncdf4::nc_close(ncf)

#starts at same date
ncf<-ncdf4::nc_open("/stacywork/hadcm3/sea_level_pressure/monthly_fixed/xnapa.nc")
...
for(ii in 1:length(DATA_past1000$CAVES$entity_info$entity_id)){
  ...
  colnames(data) = c("interp_age", "TEMP", "PREC","ISOT", "ITPC", "SLPR")
  
  DATA_past1000$CAVES$sim_data_downsampled[[nameE]] <- as.tibble(data)

}

remove(nameE, nameC, ii, data_isot, data_itpc, data, data_slpr, data_prec, data_temp)
...
remove(jj, lat, lat_band, entity_lat)
...
assign(paste0("DATA_past1000_",run))
