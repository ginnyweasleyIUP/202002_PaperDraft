  ...
  if (!is.null(gridlyr)) {
    ...
    if(!legend_cb){
      map_plot <- map_plot + scale_fill_gradientn(colors = {if (splcol) {colorscheme$grid} else {colorscheme}},
                                                  ...
                                                  labels = signif(seq(lims$grid[1], lims$grid[2], length.out = legend_num_breaks), digits = 1),
    ...
    if (!is.null(ptlyr_size_var) & !is.null(ptlyr_shape_var)) {
      ...
      map_plot <- map_plot +
        ...
        scale_size_manual(values = c(GLOBAL_STACY_OPTIONS$GLOBAL_POINT_SIZE, GLOBAL_STACY_OPTIONS$GLOBAL_POINT_SIZE / 2),
                          guide = guide_legend(ptlyr_size_var, nrow = 2, direction = 'horizontal', title.vjust = GLOBAL_STACY_OPTIONS$GLOBAL_LEG_TITLE_VJUST, order = 3))
      ...
      map_plot <- map_plot + 
        geom_point(data = ptlyr,
                   ...
                   alpha = I(0.7), shape = 21, size = GLOBAL_STACY_OPTIONS$GLOBAL_POINT_SIZE,
  ...
  if (!is.null(ptlyr)) {
    map_plot <- map_plot + 
      geom_point(data = ptlyr,
                 ...
                 alpha = I(0.7), size = GLOBAL_STACY_OPTIONS$GLOBAL_POINT_SIZE_DISCR, stroke = GLOBAL_STACY_OPTIONS$GLOBAL_POINT_STROKE,
                 show.legend = c(fill = {if (is.null(gridlyr) | splcol) {TRUE} else {FALSE}}, size = TRUE)) + 
      scale_shape_manual(values = GLOBAL_STACY_OPTIONS$GLOBAL_SHAPE_VALUES, 
                         guide = guide_legend(legend_names$pt, nrow = 3, direction = 'horizontal', title.vjust = GLOBAL_STACY_OPTIONS$GLOBAL_LEG_TITLE_VJUST,
                                              override.aes = list(size = GLOBAL_STACY_OPTIONS$GLOBAL_POINT_SIZE, alpha = I(0.7)), order = 4))
