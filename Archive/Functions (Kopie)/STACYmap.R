...
load_natural_earth_data <- function(file, dir = '~/Documents/code/STACYplots/maps/naturalearth_10m_physical', ...) {
  NAT_EARTH_PATH <- '~/Documents/code/STACYplots/maps/naturalearth_10m_physical'
...
STACYmap <- function(gridlyr = NULL, 
  ...
  # TODO datum
  # TODO graticule color, linewidth, indep of axis labels
  # TODO accessable GLOBAL VARS
  # projections
    ...
    clrgd <- function(title, ...) {
      return(
        guide_colorbar(title,
                       ...
                       title.vjust = GLOBAL_LEG_TITLE_VJUST,
    ...
    map_plot <- map_plot + 
      ...
      scale_fill_gradientn(colors = {if (splcol) {colorscheme$grid} else {colorscheme}},
                           ...
                           guide = clrgd(legend_names$grid,
                                         breaks = signif(seq(lims$grid[1], lims$grid[2], length.out = legend_num_breaks), digits = 1))) +
    ...
    if (!is.null(ptlyr_size_var) & !is.null(ptlyr_shape_var)) {
      ...
      map_plot <- map_plot + 
        geom_point(data = ptlyr,
                   ...
                   alpha = I(0.7), stroke = GLOBAL_POINT_STROKE,
                   show.legend = c(fill = {if (is.null(gridlyr) | splcol) {TRUE} else {FALSE}}, size = TRUE)) + 
        scale_shape_manual(values = c(23, 22, 24, 21, 0:20), 
                           guide = guide_legend(nrow = 2, direction = 'horizontal', title.vjust = GLOBAL_LEG_TITLE_VJUST,
                                                override.aes = list(size = GLOBAL_POINT_SIZE, fill = GLOBAL_GREY_DARK, alpha = I(0.7)), order = 4)) + 
        scale_size_manual(values = c(GLOBAL_POINT_SIZE, GLOBAL_POINT_SIZE / 2), 
                          guide = guide_legend(ptlyr_size_var, nrow = 2, direction = 'horizontal', title.vjust = GLOBAL_LEG_TITLE_VJUST, order = 3))
    } else if (!is.null(ptlyr_size_var)) {
      ptlyr[[ptlyr_size_var]] <- factor(as.character(ptlyr[[ptlyr_size_var]]), levels = c('TRUE', 'FALSE'))
      map_plot <- map_plot + 
        geom_point(data = ptlyr,
                   mapping = aes(x = long, y = lat, fill = layer, size = !!sym(ptlyr_size_var)),  
                   alpha = I(0.7),  shape = 21, stroke = GLOBAL_POINT_STROKE,
                   show.legend = c(fill = {if (is.null(gridlyr) | splcol) {TRUE} else {FALSE}}, size = TRUE)) + 
        scale_size_manual(values = c(GLOBAL_POINT_SIZE, GLOBAL_POINT_SIZE / 2), 
                          guide = guide_legend(ptlyr_size_var, nrow = 2, direction = 'horizontal', title.vjust = GLOBAL_LEG_TITLE_VJUST, order = 3))
      ...
      map_plot <- map_plot + 
        geom_point(data = ptlyr,
                   ...
                   alpha = I(0.7), size = GLOBAL_POINT_SIZE, stroke = GLOBAL_POINT_STROKE,
                   show.legend = c(fill = {if (is.null(gridlyr) | splcol) {TRUE} else {FALSE}}, size = TRUE)) + 
        scale_shape_manual(values = c(23, 22, 24, 21, 0:20), 
                           guide = guide_legend(nrow = 2, direction = 'horizontal', 
                                                override.aes = list(size = GLOBAL_POINT_SIZE, fill = GLOBAL_GREY_DARK, alpha = I(0.7)), title.vjust = GLOBAL_LEG_TITLE_VJUST, order = 3))
    } else {
      map_plot <- map_plot + 
        geom_point(data = ptlyr,
                   mapping = aes(x = long, y = lat, fill = layer),  
                   alpha = I(0.7), shape = 21, size = GLOBAL_POINT_SIZE, stroke = GLOBAL_POINT_STROKE,
                   show.legend = c(fill = {if (is.null(gridlyr) | splcol) {TRUE} else {FALSE}}, size = TRUE))
      }
...
STACYmap_discr <- function(gridlyr = NULL, 
                           ptlyr = NULL, 
                           ptlyr_size_var = NULL,
                           #ptlyr_shape_var = NULL,
                           coastline = TRUE,
  ...
  if (!is.null(ptlyr)) {
      map_plot <- map_plot + 
        geom_point(data = ptlyr,
                   mapping = aes(x = long, y = lat, fill = layer, color = layer, shape = layer),  
                   alpha = I(0.7), size = GLOBAL_POINT_SIZE_DISCR, stroke = GLOBAL_POINT_STROKE,
                   show.legend = c(fill = {if (is.null(gridlyr) | splcol) {TRUE} else {FALSE}}, size = TRUE)) + 
        scale_shape_manual(values = GLOBAL_SHAPE_VALUES, 
                           guide = guide_legend(legend_names$pt, nrow = 3, direction = 'horizontal', title.vjust = GLOBAL_LEG_TITLE_VJUST,
                                                override.aes = list(size = GLOBAL_POINT_SIZE, alpha = I(0.7)), order = 4))
    
    
    
    if (splcol | is.null(gridlyr)) {
      map_plot <- map_plot + 
        scale_fill_manual(values = colorscheme$pt, 
                            guide = guide_legend(legend_names$pt, nrow = 3, direction = 'horizontal', title.vjust = GLOBAL_LEG_TITLE_VJUST,
                                                  override.aes = list(size = GLOBAL_POINT_SIZE, alpha = I(0.7)), order = 4)) + 
        scale_color_manual(values = colorscheme$pt, 
                             guide = guide_legend(legend_names$pt, nrow = 3, direction = 'horizontal', title.vjust = GLOBAL_LEG_TITLE_VJUST,
                                                  override.aes = list(size = GLOBAL_POINT_SIZE, alpha = I(0.7)), order = 4))
    }
  }
