...
for(ii in 1:length(DATA_past1000$CAVES$entity_info$entity_id)){
  ...
  PLOTTING_DATA$FIG3$CAVElyr_isot$value[ii] = mean(DATA_past1000$CAVES$record_data[[paste0("ENTITY", entity)]]$d18O_dw_eq, na.rm = T)
}
...
Plot_lyr1 <-rbind(DATA_past1000$SIM_mean$ITPC[49:96,1:73],DATA_past1000$SIM_mean$ITPC[1:48,1:73])
...
plot  %>% ggsave(filename = paste('Paper_Plot_3_Mean', 'pdf', sep = '.'), plot = ., path = 'Plots', 
