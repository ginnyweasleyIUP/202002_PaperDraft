...
ncf<-ncdf4::nc_open("~/Dokumente/01_Promotion/06_Daten/05_HadCM3/xnapa/xnapa_westerly_u.nc")
DATA_past1000_SIM_RAW$WIND$WESTERLY <- clear_data_matrix(ncdf4::ncvar_get(ncf),4)
ncdf4::nc_close(ncf)
ncf<-ncdf4::nc_open("~/Dokumente/01_Promotion/06_Daten/05_HadCM3/xnapa/xnapa_southerly_v.nc")
...
entities_mineralogy <- sample_tb %>% filter(interp_age < 1100) %>% distinct(entity_id, mineralogy) %>% filter(entity_id != 107, entity_id != 188, entity_id != 190, entity_id != 197,
                                                                                                              entity_id != 348, entity_id != 349, entity_id != 436)
...
entities_past1000 <- sample_tb %>% filter(interp_age < 1100) %>% group_by(entity_id) %>% count() %>% 
  filter(entity_id != 107, entity_id != 188, entity_id != 190, entity_id != 197,
         entity_id != 348, entity_id != 349, entity_id != 436) %>% right_join(., entities_mineralogy, by = "entity_id") %>% filter(n>2)


sample_min2 <- sample_tb %>% filter(entity_id %in% entities_past1000$entity_id) %>% distinct(entity_id, interp_age, d18O_measurement) %>% filter(interp_age<1100)
site_min2 <- site_tb %>% filter(entity_id %in% entities_past1000$entity_id) %>% distinct(site_id, entity_id) %>% right_join(., sample_min2, by = "entity_id")
...
site_to_entity <- site_tb %>% filter(entity_id %in% entities_past1000$entity_id) %>%distinct(site_id, entity_id)

# General data for entities with more than 2 measurements
DATA_past1000$CAVES$entity_info <- entities_past1000 %>% right_join(., site_period, by = "entity_id") %>% right_join(site_to_entity,., by = "entity_id")

remove(entities_mineralogy, entities_past1000, sample_tb, site_min2, site_period, site_tb, site_to_entity, dating_tb, run)
...
for (ii in DATA_past1000$CAVES$entity_info$entity_id){
  ...
  DATA_past1000$CAVES$record_data[[name]] <- sample_min2 %>% filter(entity_id == ii) %>% distinct(entity_id, mineralogy, arag_corr, interp_age, d18O_measurement) %>%
    mutate(site_id = (site$site_id))
}

remove(data, sample_min2, site, ii, name)
...
remove(ratios, ii, lat_cave, lon_cave, name, site_id)
...
for(ii in 1:length(DATA_past1000$CAVES$entity_info$entity_id)){
  ...
  data <- matrix(c(DATA_past1000$CAVES$record_data[[nameE]]$interp_age, data_temp, data_prec, data_isot, data_itpc), ncol= 5)
  colnames(data) = c("interp_age", "Temp", "Prec","d18O", "d18O_pw")
  
  DATA_past1000$CAVES$sim_data_downsampled[[nameE]] <- as.tibble(data)

}

remove(nameE, nameC, ii, data_isot, data_itpc, data)
...
DATA_past1000$SIM_seasonal <- list(
  SUMMER = list(
    TEMP = array(dim = c(96,73,1150)), PREC = array(dim = c(96,73,1150)), ISOT = array(dim = c(96,73,1150)), ITPC = array(dim = c(96,73,1150)),
    WIND = list(STRENGTH = array(dim = c(96,72,1149)), ANGLE = array(dim = c(96,72,1149))),
    EVAP = array(dim = c(96,73,1150))),
  AUTUMN = list(
    TEMP = array(dim = c(96,73,1150)), PREC = array(dim = c(96,73,1150)), ISOT = array(dim = c(96,73,1150)), ITPC = array(dim = c(96,73,1150)),
    WIND = list(STRENGTH = array(dim = c(96,72,1149)), ANGLE = array(dim = c(96,72,1149))),
    EVAP = array(dim = c(96,73,1150))),
  WINTER = list(
    ...
    WIND = list(STRENGTH = array(dim = c(96,72,1149)), ANGLE = array(dim = c(96,72,1149))),
    EVAP = array(dim = c(96,73,1150))),
  SPRING = list(TEMP = array(dim = c(96,73,1150)), PREC = array(dim = c(96,73,1150)), ISOT = array(dim = c(96,73,1150)), ITPC = array(dim = c(96,73,1150)),
    WIND = list(STRENGTH = array(dim = c(96,72,1149)), ANGLE = array(dim = c(96,72,1149))),
    EVAP = array(dim = c(96,73,1150)))
)
