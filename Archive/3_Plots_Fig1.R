...
# SIMULATION DATA
# HadCM3 area-weighted annual mean temperature

temp <- numeric(1150)
for(ii in 1:1150){
  temp[ii] <- simpleawmean(DATA_past1000$SIM_yearly$TEMP[,,ii], seq(from = -90, to = 90, length.out = 73))
}

temp2 <- zoo(x = temp, order.by = seq(-1100, 49))

offset = mean(temp2[index(temp2)>=10 & index(temp2)< 49])

Timeseries$HadCM3_TAM <- zoo(x = temp-offset, order.by = seq(-1100, 49))

rm(temp, temp2, offset)

# HadCM3 Bunker cave --> site 117
Timeseries$HadCM3_Bunker <- zoo(x = DATA_past1000$CAVES$sim_data_yearly$CAVE117$ISOT, order.by = seq(-1100, 49))
...
alias <- 'orbital_longterm_50e4_0_1_12'
INS <- list()
...
par(mar=c(0,0,0,0),oma=c(0,12,6,13),xaxs="i",yaxs="i",cex=1,lwd=2)
...
lines(Timeseries$HadCM3_TAM,type="l",col=adjustcolor(col_hadcm3_temp,0.3))
lines(gaussdetr(Timeseries$HadCM3_TAM,tsc.in=100)$Xsmooth,type="l",col=col_hadcm3_temp)
...
mtext(side=2,expression(paste(plain(HadCM3)," ",plain(AMT))),cex=namcex,line=namlin,las=1,col=col_hadcm3_temp, at = .7)
mtext(side=2,expression(paste(plain(PAGES2k))),cex=namcex,line=namlin,las=1,col=col_pages2k, at = 0)
mtext(side=2,expression(paste(plain(HadCRUT4))),cex=namcex,line=namlin,las=1,col="black", at = -0.7)
# Add units (on right side)
mtext(side=4,"[°C] GMST",line=unitslinno,cex=unitscex,las=1,col="black")
mtext(side=4, "anomaly", line = unitslinno, cex = unitscex-0.2, las = 1, col = "black", at = -1.5)
mtext(side=4, "(1961-1990)", line = unitslinno, cex = unitscex-0.2, las = 1, col = "black", at = -2.0)

# Add axis (on right side)
axis(4,at=seq(-2,1,by=1),labels=FALSE,col="black")
mtext(side=4,at=seq(-2,1,by=1),seq(-2,1,by=1),line=axslinno,las=1,cex=axisnumscex,col="black")
...
lines(Timeseries$HadCM3_Bunker,col=adjustcolor(col_hadcm3_d18O,0.3))
lines(gaussdetr(Timeseries$HadCM3_Bunker,tsc.in=100)$Xsmooth,col=col_hadcm3_d18O)
...
axis(2,col="black",at=seq(-10,-6),labels=FALSE)
...
mtext(side=2,expression(paste("[",delta^{plain(18)}, plain(O),"]")),cex=unitscex,col="black",line=unitslinno,las=1)
mtext(side=2, "Bunker cave", cex = unitscex-0.2, col = "black", line = unitslinno, las = 1, at = -8.9)
mtext(side=4,expression(plain(HadCM3)),cex=namcex,col=col_hadcm3_d18O,las=1, line = 5, at = -7)
mtext(side=4,expression(plain(entity240)),cex=namcex,col=col_d18O_240,las=1, line = 5, at = -8)
mtext(side=4,expression(plain(entity242)),cex=namcex,col=col_d18O_242,las=1, line = 5, at = -9)
...
axis(4,col=col_co2,at=c(250,300,350,400),labels=FALSE)
mtext(side=4,col=col_co2,at=c(300,400),c(300,400),line=axslinno,las=1,cex=axisnumscex)
mtext(side=4,expression(paste("[",plain(ppm),"]")),cex=unitscex,col=col_co2,line=unitslinno,las=1)
mtext(side=2,expression(plain(CO2)),cex=namcex,col=col_co2,line=namlin,las=1)

text(-1100, 370 , "C", cex = namcex+0.5, col = col_co2)
...
mtext(side=4,expression(paste("volcanic forcing")),cex=namcex,col=col_volc,line=namlin,las=1)

text(-1100, 0.55, "D", cex = namcex+0.5, col = col_volc)

# 4: solar forcing
range_solar = c(1364, 1367.5)
#rgngrp<-range(ngrip[which(index(ngrip) <= 0 & index(ngrip) >= -67000)])
plot(xlimz,range_solar,axes=FALSE,type="n",xlab="",ylab="")
lines(Timeseries$solar,col=col_solar)


axis(4,col=col_solar,at=c(1365, 1366, 1367),labels=FALSE)
mtext(side=4,col=col_solar,at=c(1365,1366,1367),c(1365,1366,1367),line=axslinno,las=1,cex=axisnumscex)
mtext(side=4,expression(paste("[",plain(W/m^2),"]")),cex=unitscex,col=col_solar,line=unitslinno+1,las=1)
mtext(side=2,expression(paste("TSI")),cex=namcex,col=col_solar,line=namlin,las=1)

text(-1100, 1366.7 , "E", cex = namcex+0.5, col = col_solar)

# 5: orbit

range_orbit = range(1,-1)

plot(xlimz, range_orbit, axes = FALSE, type = "n", xlab = "", ylab = "")
lines(Timeseries$orbit$yrs, Timeseries$orbit$dR, col = col_orbit)

axis(4,col=col_orbit,at=c(-0.5, 0, 0.5),labels=FALSE)
...
mtext(side=2,"relavite summer",cex=namcex,col=col_orbit,line=namlin,las=1, at = 0.5)
...
#rm(namcex, namlin, num_ts, Pages_adjust, range_co2, range_d18O, range_solar, range_temp, range_volc, tmp.y, tmp.y2, unitscex, unitslinno, xlimz)
